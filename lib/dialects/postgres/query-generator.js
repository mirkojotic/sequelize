...
class PostgresQueryGenerator extends AbstractQueryGenerator {
  ...
  createTableQuery(tableName, attributes, options) {
    ...
    for (const attr in attributes) {
      ...
      if (_.includes(attributes[attr], ' COMMENT ')) {
        const commentMatch = attributes[attr].match(/^(.+) (COMMENT.*)$/);
        const commentText = commentMatch[2].replace(/COMMENT/, '').trim();
        columnComments += `; COMMENT ON COLUMN <%= table %>.${quotedAttr} IS ${this.escape(commentText)}`;
        attributes[attr] = commentMatch[1];
      }
